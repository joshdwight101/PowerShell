...
OVERVIEW
========================================
PowerShell Sync-Thing is an enterprise-friendly synchronization manager
that supports:
- One-way backup syncs (A->B or B->A)
- Two-way synchronization
- Multiple sync pairs managed in one interface
- Dynamic core detection with optional max-core limiting
- Persistent logging and settings
...
2) Action Buttons
   - + : Adds a new editable row to the grid.
   - Start Sync Server : Starts sync workers for valid rows.
   - Stop Sync Server  : Stops all active sync workers.

3) Status Console
   - Displays verbose activity including copied files, warnings, and
     lifecycle events.

========================================
SYNC MODES (PER ROW)
========================================
Sync Mode toggle cycles each time you click it:
- <--          : One-way sync from Directory Path B into Directory Path A
- -->          : One-way sync from Directory Path A into Directory Path B
- <--2-way-->  : Two-way synchronization

========================================
PATH SELECTION (LOCAL + UNC)
========================================
- Use Browse A / Browse B for convenient folder-path selection.
- UNC paths are supported (example: \\Server\Share\Folder).
- You can also manually type/paste local or UNC paths directly into the grid.

========================================
STARTING AND STOPPING
========================================
Starting:
- Click Start Sync Server.
- The app validates rows and starts workers for rows with:
  Name + Directory Path A + Directory Path B

Stopping:
- Click Stop Sync Server to halt and dispose all workers.

========================================
APPLICATION OPTIONS
========================================
Open from: Options > Application Settings

Tab: Main
- Max cores to use (0 = auto/all available)
  Use this to prevent sync workers from consuming too many CPU resources.
...
- If auto pruning is enabled and max file size is exceeded, the log is
  rotated to a .bak file with a timestamp.
- Overwrite mode writes once and then reverts to Append.

========================================
PERSISTENCE
========================================
- Settings and sync pair definitions are persisted to:
  %APPDATA%\PowerShellSyncThing\settings.json
- Settings are loaded on startup and saved when options are saved and on exit.

========================================
BEST PRACTICES
========================================
- Prefer stable network paths for UNC shares.
- Use one-way mode for strict backup pipelines.
- Use two-way mode only when bidirectional changes are expected.
- Set a Max cores limit in busy environments to reduce system impact.

========================================
TROUBLESHOOTING
========================================
- If a source path is missing, warnings appear in the status console.
- Verify folder permissions for both directories.
- For network shares, confirm connectivity and access rights.
- Review log output for detailed synchronization actions.
"@
    $manualForm = New-Object System.Windows.Forms.Form
    $manualForm.Text = 'Manual - PowerShell Sync-Thing'
    $manualForm.Size = New-Object System.Drawing.Size(950,700)
    $manualForm.StartPosition = 'CenterParent'

    $manualHeader = New-Object System.Windows.Forms.Label
    $manualHeader.Location = '15,12'; $manualHeader.Size = '880,26'
    $manualHeader.Font = New-Object System.Drawing.Font('Segoe UI',11,[System.Drawing.FontStyle]::Bold)
    $manualHeader.Text = "PowerShell Sync-Thing User Manual (v$script:AppVersion)"

    $manualBox = New-Object System.Windows.Forms.TextBox
    $manualBox.Location = '15,45'; $manualBox.Size = '900,560'
    $manualBox.Multiline = $true
    $manualBox.ReadOnly = $true
    $manualBox.ScrollBars = 'Vertical'
    $manualBox.Font = New-Object System.Drawing.Font('Consolas',10)
    $manualBox.Text = $manualText
    $manualBox.Anchor = 'Top,Bottom,Left,Right'

    $manualClose = New-Object System.Windows.Forms.Button
    $manualClose.Location = '820,615'; $manualClose.Size = '95,30'; $manualClose.Text = 'Close'
    $manualClose.Anchor = 'Bottom,Right'
    $manualClose.add_Click({ $manualForm.Close() })

    $manualForm.Controls.AddRange(@($manualHeader,$manualBox,$manualClose))
